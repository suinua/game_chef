...
{
    public function __construct(Player $player, string $mapName, Vector3 $spawnPoint) {
        parent::__construct($player,
            [
                new HotbarMenuItem(ItemIds::FEATHER, "戻る", function () {
                    $this->close();
                }),
                new HotbarMenuItem(ItemIds::TNT, "削除", function (Player $player) use ($mapName, $spawnPoint) {
                    try {
                        FFAGameMapService::deleteSpawnPoint($mapName, $spawnPoint);
